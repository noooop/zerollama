...
class SuryaTextLineDetection(TextLineDetectionModel):
    family = "surya_tld"
    header = ["name"]
    info = [
        ["surya_tld"]
    ]
    inference_backend = "zerollama.models.surya.backend.text_line_detection:SuryaTextLineDetection"
...
if __name__ == '__main__':
    ...
    for model_name in [x[0] for x in SuryaTextLineDetection.info]:
        model = SuryaTextLineDetection.get_model(model_name)
        model.load()
