...
<img src="https://github.com/noooop/noooop.github.io/blob/main/benchmarking/prefill/prefill-gguf-50.png?raw=true" width="800">

- a. 提示词长度小于 8，相同模型的不同量化版本，速度差异不大，可以认为是 llama.cpp 起步延迟
- b. 提示词长度 8-32 之间，相同模型的不同量化版本，速度差异很大

> 以 14B 模型，提示词长度 20 为例:
> 
> | 排名   | 1        | 2        | 3        | 4        | 5        | 6        | 7        | 8        |
> |------|----------|----------|----------|----------|----------|----------|----------|----------|
> | 量化方法 | q5_k_m   | q8_0     | q5_0     | q4_k_m   | q2_k     | q4_0     | q3_k_m   | q6_k     |
> | 延迟   | 29.95 ms | 36.51 ms | 41.79 ms | 44.50 ms | 44.50 ms | 48.38 ms | 49.82 ms | 56.88 ms |
> 
> - q5_k_m 最快，q6_k 最慢， Latency 排名基本上跟 BPW 没啥关系
> - 可能是不同量化版本计算和带宽比不同，后面会具体测试一下

- c. 32 之后，相同模型的不同量化版本又基本相同

<img src="https://github.com/noooop/noooop.github.io/blob/main/benchmarking/prefill/prefill-14B-1.png?raw=true" width="800">

> 以 14B 模型提示词长度 100 为例 
> 
> | name   | BPW  | Latency |
> |--------|------|---------|
> | q8_0   | 8.5  | 67.85   |
...
> | q4_k_m | 5.19 | 72.69   |
> | q2_k   | 3.43 | 74.12   |
> - Latency 排名基本上跟 BPW 一致

<img src="https://github.com/noooop/noooop.github.io/blob/main/benchmarking/prefill/prefill-gguf-500.png?raw=true" width="800">

- d. 更大尺度上，32 之后 也趋于稳定
- 进一步探究 GGUF 预填充 (Prefill) 阶段速度曲线，需要使用NVIDIA nsight systems profiling工具

### 6.5.2 llama.cpp nsight systems profiling
以14B模型为例

<img src="https://github.com/noooop/noooop.github.io/blob/main/benchmarking/prefill/nsys.png?raw=true" width="800">

- 找到 memcpy HtoD 这是一次推理的开始
- 找到最大的矩阵乘法， 这个是算最后 lm_head，差不多是一次推理的结束
- llm 线性层运算（比如kqv mlp等），底层是矩阵乘法，其实现速度决定了整体速度
- 图里是 序列长度等于1 的时候，使用 mul_mat_vec_q<(int)1 实现矩阵乘法，一次大概 30 μs

<img src="https://github.com/noooop/noooop.github.io/blob/main/benchmarking/prefill/nsys-14B-q8_0.png?raw=true" width="800">

- 以 q8_0 模型为例，看不同序列长度是怎么实现的
- 长度为1-8，使用了矩阵向量乘法， mul_mat_vec_q<(int)1, (int)32, (int)8, block_q8_0, (int)2, &vec_dot_q8_0_q8_1> ~ mul_mat_vec_q<(int)8, (int)32, (int)8, block_q8_0, (int)2, &vec_dot_q8_0_q8_1>
- 9-32 （大部分层）使用了 mul_mat_q8_0
- 32 以上使用 dequantize_block_q8_0_f16+ampere_h16816gemm_64x128_ldg8_stages_32x6_tn 实现
- 小长度融合解量化和矩阵乘法，长度比较大时（大于32时）先解量化到f16，再使用f16的gemm计算
- 真的 amazing

### 6.5.3 llama.cpp 进一步 nsight systems profiling

- GGUF 使用了混合 精度量化，不同层使用了不同的量化方法，所以整个推理流程使用多种 kernel
- 以下14B模型是典型层的典型速度 单位 μs

| 序列长度                   | q8_0       | q6_k       | q5_k_m     | q5_0       | q4_k_m     | q4_0       | q3_k_m     | q2_k       |
|------------------------|------------|------------|------------|------------|------------|------------|------------|------------|
| 实现 <br/> mul_mat_vec_q | block_q8_0 | block_q6_K | block_q5_K | block_q5_0 | block_q4_K | block_q4_0 | block_q3_K | block_q2_K |
| 1                      | 30.56      | 23.90      | 20.32      | 20.26      | 17.31      | 16.86      | 14.05      | 10.82      |
| 2                      | 31.14      | 24.42      | 21.12      | 20.83      | 17.86      | 17.47      | 16.32      | 12.70      |
| 3                      | 31.65      | 24.42      | 21.57      | 20.96      | 18.56      | 17.31      | 20.32      | 14.40      |
| 4                      | 31.52      | 25.28      | 22.84      | 21.95      | 20.42      | 17.98      | 21.92      | 18.40      |
| 5                      | 31.30      | 27.55      | 24.74      | 22.69      | 22.46      | 20.77      | 21.50      | 18.82      |
| 6                      | 31.87      | 27.90      | 26.30      | 23.75      | 24.42      | 21.51      | 25.22      | 21.47      |
| 7                      | 31.94      | 30.11      | 27.26      | 24.83      | 27.68      | 22.37      | 27.26      | 24.29      |
| 8                      | 35.87      | 32.00      | 31.90      | 25.76      | 29.86      | 24.29      | 32.22      | 28.29      |

- 1-8长度区间使用融合了解量化的mul_mat_vec_q实现
- 速度整体跟BPW排序一致
- 序列短， 1-2，完全跟 BPW 排序一致
- 序列越长，q4_k_m 和 q3_k_m 就是比左右邻居慢，估计是混合精度要调用多种kernel，开销大一点

| 序列长度 | q8_0         | q6_k         | q5_k_m       | q5_0         | q4_k_m       | q4_0         | q3_k_m       | q2_k         |
|------|--------------|--------------|--------------|--------------|--------------|--------------|--------------|--------------|
| 实现   | mul_mat_q8_0 | mul_mat_q6_K | mul_mat_q5_K | mul_mat_q5_0 | mul_mat_q4_K | mul_mat_q4_0 | mul_mat_q3_K | mul_mat_q2_K |
| 9    | 46.30        | 58.24        | 55.58        | 65.47        | 45.70        | 29.57        | 80.45        | 29.82        |
| 10   | 47.33        | 58.30        | 56.80        | 61.86        | 41.98        | 27.52        | 80.64        | 27.49        |
| 11   | 46.72        | 58.72        | 56.26        | 61.86        | 41.86        | 30.88        | 80.13        | 29.76        |
| 12   | 45.89        | 58.46        | 56.61        | 61.41        | 42.18        | 27.33        | 80.77        | 27.65        |
| 13   | 53.57        | 70.88        | 75.49        | 64.70        | 50.88        | 33.60        | 96.48        | 33.83        |
| 14   | 54.30        | 71.07        | 68.61        | 65.63        | 51.68        | 33.60        | 97.09        | 36.48        |
| 15   | 54.40        | 70.50        | 68.10        | 65.19        | 51.33        | 33.63        | 96.71        | 33.28        |
| 16   | 54.40        | 70.69        | 68.90        | 64.93        | 51.46        | 36.83        | 96.77        | 33.92        |
| 17   | 73.34        | 95.75        | 93.92        | 77.28        | 70.63        | 45.18        | 129.31       | 44.13        |
| 18   | 73.54        | 95.87        | 93.41        | 77.15        | 70.15        | 45.18        | 129.89       | 44.29        |
| 19   | 73.25        | 95.46        | 93.44        | 77.06        | 70.18        | 45.22        | 129.86       | 44.26        |
| 20   | 74.18        | 96.26        | 92.99        | 77.12        | 69.86        | 48.93        | 129.70       | 44.39        |
| 21   | 83.33        | 108.87       | 105.79       | 89.38        | 79.07        | 51.04        | 147.17       | 49.86        |
| 22   | 83.68        | 107.68       | 105.60       | 88.42        | 79.84        | 51.49        | 146.85       | 50.24        |
| 23   | 83.55        | 108.80       | 105.70       | 89.02        | 80.29        | 51.42        | 147.11       | 50.82        |
| 24   | 83.20        | 108.58       | 105.63       | 91.87        | 79.30        | 51.23        | 147.94       | 50.46        |
| 25   | 93.41        | 120.35       | 117.99       | 101.92       | 89.12        | 57.57        | 165.35       | 57.18        |
| 26   | 92.03        | 120.67       | 117.34       | 102.11       | 88.42        | 57.18        | 163.36       | 57.25        |
| 27   | 92.45        | 120.71       | 117.86       | 101.70       | 88.74        | 57.99        | 163.87       | 57.25        |
| 28   | 92.13        | 121.57       | 118.21       | 100.77       | 89.06        | 57.63        | 163.33       | 56.38        |
| 29   | 101.63       | 145.09       | 142.98       | 139.30       | 98.34        | 68.64        | 181.12       | 69.54        |
| 30   | 101.89       | 146.18       | 143.01       | 140.10       | 98.02        | 69.02        | 180.42       | 67.30        |
| 31   | 102.72       | 146.40       | 143.01       | 140.74       | 98.14        | 68.86        | 182.59       | 67.04        |
| 32   | 101.86       | 145.22       | 142.24       | 139.04       | 98.91        | 69.38        | 180.67       | 67.39        |

- 9-32长度区间使用融合了解量化的mul_mat实现
- 速度差异，来源于矩阵乘法实现差异，差别还是很大，甚至有一倍的差距
- 每四个序列长度速度基本一样，应该是内存对齐的影响
> - 仔细对比，可以发现这个表和之前"提示词长度等于20"的表排序不一样
> - 因为哪怕是一个序列长度，不同量化模型的不同层也使用了不同的计算方式
> - 不想把事情搞的太复杂，记各种层的流水账也没啥意思

| 序列长度          | q8_0                      | q6_k                          | q5_k_m                        | q5_0                                                        | q4_k_m                        | q4_0                          | q3_k_m                        | q2_k                          |
|---------------|---------------------------|-------------------------------|-------------------------------|-------------------------------------------------------------|-------------------------------|-------------------------------|-------------------------------|-------------------------------|
| dequantize 实现 | dequantize_block_q8_0_f16 | dequantize_block_q6_K<__half> | dequantize_block_q5_K<__half> | dequantize_block<(int)32, (int)2, &dequantize_q5_0, __half> | dequantize_block_q4_K<__half> | dequantize_block_q4_0<__half> | dequantize_block_q3_K<__half> | dequantize_block_q2_K<__half> |
| 33            | 50.847+38.785             | 49.728+37.152                 | 49.920+37.888                 | 54.624+34.272                                               | 49.152+33.504                 | 49.152+34.816                 | 55.360+33.376                 | 48.865+33.824                 |

- 长度 33 以后，矩阵乘法使用 dequantize+gemm 实现
- 对比dequantize、gemm两个阶段，各种量化方法的差距比较小，不像9-32区间甚至有一倍的差距
- 对比序列长度 32 和 33，有些 mul_mat 快，有些 dequantize+gemm，显然还能再改进 （狗头）

最后：
- 想要利用最佳的矩阵乘法实现方法，需要 benchmark 每种模型，每种卡，每种序列长度，选择最快的实现方式
- 不至于，不至于
