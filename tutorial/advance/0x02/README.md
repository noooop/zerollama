...
| 模型大小 | 模型参数量 | kv cache 参数量（每词） | w16kv16 | w8kv16 | w4kv16 | 
|------|-------|------------------|---------|--------|--------|
...
| 模型大小 | DecoderLayers 参数量 | lm_head 参数量 | 模型总（激活）参数量 | 读取延迟 w16kv16 | 读取延迟 w8kv16 | 计算延迟 粗略 | 计算延迟 精细 | FTL w8kv16 精细 | 
|------|-------------------|-------------|------------|--------------|-------------|---------|---------|---------------|
...
| 模型大小 | DecoderLayers 参数量 | lm_head 参数量 | 模型总（激活）参数量 | 读取延迟 w16kv16 | 读取延迟 w8kv16 | 计算延迟 粗略 | 计算延迟 精细 | FTL w8kv16 精细 长度=20 | FTL w8kv16 精细 长度=1 |  
|------|-------------------|-------------|------------|--------------|-------------|---------|---------|---------------------|--------------------|
...
| 模型大小       | 模型参数量 | kv cache 参数量（每词） | w16kv16 | w8kv16 | w4kv16 | w6kv16 | w4kv8  | 
|------------|-------|------------------|---------|--------|--------|--------|--------|
...
除了模型可以量化，kv cache 也可以量化, 比如[KVQuant](https://arxiv.org/abs/2401.18079)、[W4A8KV4](https://arxiv.org/abs/2405.04532)。

收益也非常明显，降低浮点占用意味着成倍的提高响应时间，上下文长度。
对于访存带宽限制的llm，只要精巧的写 cuda kernel 将计算阶段量化和反量化融合进去，总可以被访存时间 cover 住。

剩下就是精度和速度的权衡。 简单建议，w6kv16是现在最好的量化方法
...
# 6. HuggingFace Transformers 库实际推理速度测试

# 7. llama.cpp 库实际推理速度测试
