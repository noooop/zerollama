...
| 模型大小 | DecoderLayers 参数量 | lm_head 参数量 | 激活参数量  | 读取延迟 w16kv16 | 计算延迟 精细 | 计算延迟 粗略 | 首字延迟 w16kv16 精细 | 
|------|-------------------|-------------|--------|--------------|---------|---------|-----------------|
| 0.5B | 0.29B             | 0.14B       | 0.43B  | 0.86ms       | 0.01ms  | 0.01ms  | 0.86ms          |
| 1.8B | 1.13B             | 0.29B       | 1.42B  | 2.82ms       | 0.03ms  | 0.03ms  | 2.82ms          |
| 4B   | 2.95B             | 0.36B       | 3.32B  | 6.58ms       | 0.08ms  | 0.08ms  | 6.58ms          |
| 7B   | 6.03B             | 0.58B       | 6.61B  | 13.12ms      | 0.16ms  | 0.16ms  | 13.12ms         |
| 14B  | 11.74B            | 0.73B       | 12.47B | 24.74ms      | 0.29ms  | 0.29ms  | 24.74ms         |
| 72B  | 65.00B            | 1.16B       | 66.16B | 131.28ms     | 1.56ms  | 1.56ms  | 131.28ms        |

使用 w8kv16， 模型大小降低一半，仍然卡在带宽上

| 模型大小 | 读取延迟 w16kv16 | 读取延迟 w8kv16 | 计算延迟 精细 | 
|------|--------------|-------------|---------|
| 0.5B | 0.86ms       | 0.43ms      | 0.01ms  |
| 1.8B | 2.82ms       | 1.41ms      | 0.03ms  |
| 4B   | 6.58ms       | 3.29ms      | 0.08ms  |
| 7B   | 13.12ms      | 6.56ms      | 0.16ms  |
| 14B  | 24.74ms      | 12.37ms     | 0.29ms  |
| 72B  | 131.28ms     | 65.64ms     | 1.56ms  |


prompt_tokens 长度 = 1000， 卡在算力上

| 模型大小 | 激活参数量  | 读取延迟 w16kv16 | 读取延迟 w8kv16 | 计算延迟 粗略   | 计算延迟 精细   | 首字延迟 w16 kv16 精细 | 首字延迟 w8kv16 精细 | 
|------|--------|--------------|-------------|-----------|-----------|------------------|----------------|
| 0.5B | 0.43B  | 1.04ms       | 0.61ms      | 10.22ms   | 6.80ms    | 10.22ms          | 6.80ms         |
| 1.8B | 1.42B  | 3.18ms       | 1.77ms      | 33.61ms   | 26.76ms   | 33.61ms          | 26.76ms        |
| 4B   | 3.32B  | 7.34ms       | 4.05ms      | 78.45ms   | 69.89ms   | 78.45ms          | 69.89ms        |
| 7B   | 6.61B  | 14.09ms      | 7.53ms      | 156.38ms  | 142.68ms  | 156.38ms         | 142.68ms       |
| 14B  | 12.47B | 26.25ms      | 13.88ms     | 294.92ms  | 277.79ms  | 294.92ms         | 277.79ms       |
| 72B  | 66.16B | 136.12ms     | 70.48ms     | 1564.88ms | 1537.46ms | 1564.88ms        | 1537.46ms      |
...
- 拐点跟模型参数保存类型有关，占的空间越小，拐点来的越早。从w16kv16的84左右到w4kv16的21左右。
- 拐点明显小于 hidden_size，所以不用考虑超长的 prompt_tokens 在算 sdpa 时的权衡。

再 对比一组，prompt_tokens 长度 = 1 vs 20 验证一下拐点内延迟基本不变

| 模型大小 | 首字延迟 w16kv16 精细 长度=20 | 首字延迟 w16kv16 精细 长度=1 | 首字延迟 w8kv16 精细 长度=20 | 首字延迟 w8kv16 精细 长度=1 |  
|------|-----------------------|----------------------|----------------------|---------------------|
| 0.5B | 0.86ms                | 0.86ms               | 0.43ms               | 0.43ms              |
| 1.8B | 2.83ms                | 2.82ms               | 1.42ms               | 1.41ms              |
| 4B   | 6.60ms                | 6.58ms               | 3.31ms               | 3.29ms              |
| 7B   | 13.14ms               | 13.12ms              | 6.58ms               | 6.56ms              |
| 14B  | 24.77ms               | 24.74ms              | 12.40ms              | 12.37ms             |
| 72B  | 131.37ms              | 131.28ms             | 65.73ms              | 65.64ms             |
...
前沿的量化论文已经讨论2bits甚至[1-bit](https://arxiv.org/abs/2402.17764)。
...
预填充 (Prefill) 阶段 带宽瓶颈到算力瓶颈拐点 单位 ms

| 模型                     | 输入25个token | 输入26个token | 输入27个token | 输入27个token理论 | 输入27个token实际速度比 | 
|------------------------|------------|------------|------------|--------------|-----------------|
| Qwen/Qwen1.5-0.5B-Chat | 6.07 ms    | 6.04 ms    | 6.06 ms    | 0.86 ms      | 14%             |
| Qwen/Qwen1.5-1.8B-Chat | 7.18 ms    | 7.16 ms    | 6.92 ms    | 2.83 ms      | 41%             |
| Qwen/Qwen1.5-4B-Chat   | 13.67 ms   | 13.24 ms   | 13.25 ms   | 6.60 ms      | 50%             |
| Qwen/Qwen1.5-7B-Chat   | 20.64 ms   | 20.64 ms   | 20.64 ms   | 13.14 ms     | 64%             |

- 因为有启动开销，输入短的时候，实际速度跟理论差距比较大
- 大模型比小模型更接近理论值

| 模型                     | 输入100个token | 输入150个token | 输入200个token | 输入200个token理论(粗略/精细) | 输入200个token实际速度比 | 
|------------------------|-------------|-------------|-------------|----------------------|------------------|
| Qwen/Qwen1.5-0.5B-Chat | 6.15 ms     | 6.93 ms     | 6.71 ms     | 1.36ms / 2.04ms      | 20% / 30%        |
| Qwen/Qwen1.5-1.8B-Chat | 7.86 ms     | 9.87 ms     | 10.06 ms    | 5.36ms / 6.72ms      | 53% / 67%        |
| Qwen/Qwen1.5-4B-Chat   | 14.60 ms    | 19.74 ms    | 20.37 ms    | 13.99ms / 15.69ms    | 69% / 77%        |
| Qwen/Qwen1.5-7B-Chat   | 23.1 ms     | 31.56 ms    | 29.90 ms    | 28.55ms / 31.28ms    | 96% / 100%       |

- 输入长的时候，实际速度跟理论差距明显变小
- 大模型比如 7B 接近理论值
- 计算一次 lm_head 和 计算多次 lm_head， 也就是 粗略/精细 在小模型上差别很大

| 模型大小 | 拐点 w16kv16 粗略 | 拐点 w16kv16 精细 | 实际拐点 |
|------|---------------|---------------|------|
| 0.5B | 85.38         | 129.11        | 123  |
| 1.8B | 84.79         | 106.55        | 129  |
| 4B   | 84.69         | 95.07         | 130  |
| 7B   | 84.40         | 92.47         | 129  |

- 虽然跟推导的差距比较大，可能是因为开始时理论速度跟实际速度差别比较大，但确实存在一个拐点。

解码 （Decoding） 阶段 推理速度 单位

理论速度

| 模型                     | 128     | 256     | 384     | 512     | 640      | 768      | 896      | 1024     | 1152     | 1280     |
|------------------------|---------|---------|---------|---------|----------|----------|----------|----------|----------|----------|
| Qwen/Qwen1.5-0.5B-Chat | 606.23  | 1366.33 | 2143.14 | 2902.48 | 3678.73  | 4450.20  | 5194.44  | 5958.64  | 6767.91  | 7504.97  |
| Qwen/Qwen1.5-1.8B-Chat | 644.70  | 1450.54 | 2262.77 | 3075.21 | 3900.44  | 4726.12  | 5576.10  | 6400.81  | 7243.77  | 8111.40  |
| Qwen/Qwen1.5-4B-Chat   | 1241.93 | 2815.10 | 4407.80 | 6029.43 | 7659.49  | 9307.44  | 10980.64 | 12671.53 | 14376.86 | 16097.89 |
| Qwen/Qwen1.5-7B-Chat   | 1932.30 | 4368.41 | 6828.67 | 9306.68 | 11820.93 | 14346.04 | 16903.01 | 19474.45 | 22061.93 | 24671.26 |

实际速度

| 模型                     | 128     | 256     | 384     | 512     | 640     | 768      | 896      | 1024     | 1152     | 1280     |
|------------------------|---------|---------|---------|---------|---------|----------|----------|----------|----------|----------|
| Qwen/Qwen1.5-0.5B-Chat | 117.93  | 264.49  | 411.04  | 557.60  | 704.16  | 850.71   | 997.27   | 1143.82  | 1290.38  | 1436.94  |
| Qwen/Qwen1.5-1.8B-Chat | 349.63  | 784.11  | 1218.59 | 1653.07 | 2087.55 | 2522.03  | 2956.51  | 3390.99  | 3825.47  | 4259.96  |
| Qwen/Qwen1.5-4B-Chat   | 751.93  | 1686.34 | 2620.75 | 3555.17 | 4489.58 | 5424.0   | 6358.41  | 7292.83  | 8227.24  | 9161.65  |
| Qwen/Qwen1.5-7B-Chat   | 1469.67 | 3296.02 | 5122.38 | 6948.74 | 8775.10 | 10601.46 | 12427.81 | 14254.17 | 16080.53 | 17906.89 |

速度比

| 模型                     | 128 | 256  | 384 | 512 | 640 | 768 | 896 | 1024 | 1152 | 1280 |
|------------------------|-----|------|-----|-----|-----|-----|-----|------|------|------|
| Qwen/Qwen1.5-0.5B-Chat | 19% | 19%  | 19% | 19% | 19% | 19% | 19% | 19%  | 19%  | 19%  |
| Qwen/Qwen1.5-1.8B-Chat | 54% | 54%  | 53% | 53% | 53% | 53% | 53% | 52%  | 52%  | 52%  |
| Qwen/Qwen1.5-4B-Chat   | 60% | 59%  | 59% | 58% | 58% | 58% | 57% | 57%  | 57%  | 56%  |
| Qwen/Qwen1.5-7B-Chat   | 76% | 754% | 75% | 74% | 74% | 73% | 73% | 73%  | 72%  | 72%  |

- 小模型跟理论差距大一些，大模型差距小一些

### 6.1.2. 预填充 (Prefill) 阶段 带宽瓶颈到算力瓶颈拐点
