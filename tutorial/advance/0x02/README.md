...
其中 32B 和 110B 使用了 GQA，计算稍有改变，会在 [GQA](./#自动配置) 一节详细讨论。
...
| 模型大小 | DecoderLayers 参数量 | lm_head 参数量 | 模型总（激活）参数量 | 读取延迟 w16a16 | 读取延迟 w8a16 | 计算延迟 粗略 | 计算延迟 精细 | FTL w8a16 精细 | FTL w8a16 精细 长度=1 |  
|------|-------------------|-------------|------------|-------------|------------|---------|---------|--------------|-------------------|
...
> [Speculative Decoding](https://arxiv.org/abs/2211.17192) 先不考虑
