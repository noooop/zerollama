...
| 模型   | w8kv16理论（对照） | w4kv16理论（对照） | bfloat16 | bnb int8 | bnb int4 | GPTQ-Int4 | GPTQ-Int4 <br/>use_marlin | AWQ gemm <br/> fuse_layers False | AWQ use_exllama_v2 <br/> fuse_layers False | AWQ use_exllama <br/> fuse_layers False |
|------|--------------|--------------|----------|----------|----------|-----------|---------------------------|----------------------------------|--------------------------------------------|-----------------------------------------|
| 0.5B | 257424       | 259784       | 10W      | 10W      | 10W      | 10W       | 10W                       | 10W                              | 10W                                        | 10W                                     |
| 1.8B | 123312       | 127192       | 49131    | 52211    | 53738    | 53255     | Exception                 | 53763                            | 53765                                      | 53767                                   |
| 4B   | 54220        | 58567        | 18435    | 22530    | 24578    | 24167     | Exception                 | 24578                            | 24168                                      | 24577                                   |
| 7B   | 35612        | 42382        | 7683     | 13827    | 16642    | 15873     | Exception                 | 16642                            | 16385                                      | 16386                                   |
| 14B  | 15114        | 23285        | oom      | 4507     | 7785     | 7374      | Exception                 | 8194                             | 7988                                       | 8193                                    |
| 32B  | oom          | 37778        | oom      | oom      | 3450     | 2050      | Exception                 | 2658                             | oom                                        | 2562                                    |
...
- 当使用 awq 使用 fuse_layers True 的时候，极限推理长度 可以增加到离谱的10W，也就是没测出极限来？？？是使用内存做缓存了吗，出bug了吗？
...
5. 模型占内存空间的大小跟 BPW (bits per weight) 有关，需要做换算对比才算公平。
...
- GPTQ int4 Marlin 稍比 exllamav2 慢一些，7千 token 前比 bfloat16 快
- 因为后续模型加载都报错，我只能暂时得出结论，Marlin 不如 exllamav2

<img src="https://github.com/noooop/noooop.github.io/blob/main/benchmarking/decoding/0.5B-4.png?raw=true" width="800">

- 接下来是有6名选手的 AWQ 量化方法 (实际上 AWQ 还支持静态分配 cache，所以有12名选手，后续能拉开区别的时候会加入对比)
...
- llama.cpp 推理还是非常稳定、丝滑。（虽然速度不太快，后话
...
- bnb int8就不提了，int4速度也不太行，每4百左右会有一个毛刺，估计是在重新分配显存
- bfloat16 开始速度比 GGUF 系列都快，但增速也陡，到5千左右，速度就比 GGUF 系列慢，看来 HuggingFace Transformers 的 kv cache 开销比较大
- GPTQ int4 exllamav2 速度比 bfloat16 快，但考虑到 int4 模型比 bfloat16 小四倍，表现并不惊奇
- GPTQ int4 exllamav2 和 bfloat16 序列长度超过三千后，斜率差不多，kv cache相关的估计是运行的同一套代码
...
- 不使用 fuse_layers
- GPTQ int4 exllamav2 跟 AWQ exllamav2 曲线几乎重合
- AWQ GEMM 比他们俩稍微慢一点
- AWQ exllama 两千以前 跟 AWQ exllamav2 曲线几乎重合，甚至还快一些，两千以后拉开距离
- 使用 fuse_layers 后，AWQ GEMM 最慢，AWQ exllama 第二，AWQ exllamav2 最快
...
- 理论跟实际差别在缩小
- llama.cpp 表现稳健、丝滑，HuggingFace Transformers 做推理有很多毛刺
- AWQ ExllamaV2 fuse_layers 表现亮眼
- bnb int8 不可用，int4 速度也差，不太行
...
| 名称                             | BPW  | Prefill  | W    | B        |
|--------------------------------|------|----------|------|----------|
| 理论 w16kv16                     | 16   |          | 2642 | 6.58 ms  |
| q8_0.gguf                      | 8.5  | 27.18 ms | 1967 | 11.72 ms |
| q6_k.gguf                      | 6.57 | 22.12 ms | 1961 | 10.81 ms |
| q5_k_m.gguf                    | 5.74 | 33.54 ms | 1966 | 10.45 ms |
| q5_0.gguf                      | 5.61 | 28.6 ms  | 1964 | 10.37 ms |
| q4_k_m.gguf                    | 4.96 | 30.49 ms | 1958 | 10.07 ms |
| q4_0.gguf                      | 4.71 | 27.46 ms | 1959 | 9.9 ms   |
| q3_k_m.gguf                    | 4.09 | 43.31 ms | 1955 | 9.82 ms  |
| q2_k.gguf                      | 3.27 | 49.74 ms | 1954 | 9.44 ms  |
| bfloat16                       | 16   | 4.86 ms  | 731  | 11.55 ms |
| bnb int8                       | 8    | 6.63 ms  | 822  | 75.75 ms |
| bnb int4                       | 4    | 7.63 ms  | 3446 | 17.96 ms |
| GPTQ-Int4 <br/> exllamav2 （默认) | 4    | 7.75 ms  | 798  | 7.34 ms  |
| max_seq_len yes                |      |          |      |          |
| AWQ <br/> GEMM （默认)            | 4    | 13.30 ms | 893  | 9.82 ms  |
| AWQ <br/> GEMM                 | 4    | 6.46 ms  | 1492 | 5.93 ms  |
| AWQ <br/> Exllama              | 4    | 5.75 ms  | 747  | 8.05 ms  |
| AWQ <br/> Exllama              | 4    | 5.63 ms  | 1471 | 6.92 ms  |
| AWQ <br/> ExllamaV2            | 4    | 5.78 ms  | 787  | 4.61 ms  |
| AWQ <br/> ExllamaV2            | 4    | 5.85 ms  | 1492 | 4.66 ms  |
| max_seq_len no                 |      |          |      |          |
| AWQ <br/> GEMM （默认)            | 4    | 9.01 ms  | 860  | 9.75 ms  |
| AWQ <br/> GEMM                 | 4    | 6.40 ms  | 6867 | 7.40 ms  |
| AWQ <br/> Exllama              | 4    | 7.63 ms  | 743  | 7.99 ms  |
| AWQ <br/> Exllama              | 4    | 5.33 ms  | 6592 | 7.56 ms  |
| AWQ <br/> ExllamaV2            | 4    | 5.95 ms  | 787  | 6.95 ms  |
| AWQ <br/> ExllamaV2            | 4    | 5.76 ms  | 6781 | 6.16 ms  |
...
| 名称                             | BPW  | Prefill  | W    | B        |
|--------------------------------|------|----------|------|----------|
| 理论 w16kv16                     | 16   |          | 2064 | 13.12 ms |
| q8_0.gguf                      | 8.5  | 36.47 ms | 1614 | 15.28 ms |
| q6_k.gguf                      | 6.56 | 28.75 ms | 1613 | 13.52 ms |
| q5_k_m.gguf                    | 5.72 | 41.03 ms | 1609 | 12.76 ms |
| q5_0.gguf                      | 5.59 | 38.59 ms | 1601 | 12.6 ms  |
| q4_k_m.gguf                    | 4.93 | 36.98 ms | 1606 | 11.98 ms |
| q4_0.gguf                      | 4.67 | 33.14 ms | 1605 | 11.72 ms |
| q3_k_m.gguf                    | 4.05 | 53.4  ms | 1620 | 11.26 ms |
| q2_k.gguf                      | 3.21 | 61.33 ms | 1603 | 10.49 ms |
| bfloat16                       | 16   | 4.54 ms  | 533  | 18.13 ms |
| bnb int8                       | 8    | 6.77 ms  | 512  | 60.53 ms |
| bnb int4                       | 4    | 7.57 ms  | 688  | 12.86 ms |
| GPTQ-Int4 <br/> exllamav2 （默认) | 4    | 7.20 ms  | 553  | 7.95 ms  |

| 名称                  | BPW | Prefill | W    | B       |
|---------------------|-----|---------|------|---------|
| AWQ <br/> GEMM （默认) | 4   | 6.63 ms | 548  | 9.14 ms |
| AWQ <br/> GEMM      | 4   | 6.13 ms | 1552 | 7.59 ms |
| AWQ <br/> Exllama   | 4   | 5.43 ms | 541  | 8.47 ms |
| AWQ <br/> Exllama   | 4   | 5.75 ms | 1566 | 7.05 ms |
| AWQ <br/> ExllamaV2 | 4   | 5.10 ms | 552  | 7.71 ms |
| AWQ <br/> ExllamaV2 | 4   | 5.88 ms | 1568 | 6.59 ms |

| 名称                  | BPW | Prefill | W    | B       |
|---------------------|-----|---------|------|---------|
| AWQ <br/> GEMM （默认) | 4   | 9.87 ms | 549  | 9.18 ms |
| AWQ <br/> GEMM      | 4   | 6.20 ms | 1555 | 7.59 ms |
| AWQ <br/> Exllama   | 4   | 7.53 ms | 542  | 8.52 ms |
| AWQ <br/> Exllama   | 4   | 5.82 ms | 1546 | 7.30 ms |
| AWQ <br/> ExllamaV2 | 4   | 8.31 ms | 553  | 7.74 ms |
| AWQ <br/> ExllamaV2 | 4   | 5.67 ms | 1564 | 6.60 ms |

max_seq_len no

| 名称                  | BPW | Prefill | W    | B       |
|---------------------|-----|---------|------|---------|
| AWQ <br/> GEMM （默认) | 4   | 6.96 ms | 548  | 9.19 ms |
| AWQ <br/> GEMM      | 4   | 6.37 ms | 8181 | 8.87 ms |
| AWQ <br/> Exllama   | 4   | 5.82 ms | 542  | 8.48 ms |
| AWQ <br/> Exllama   | 4   | 5.47 ms | 8205 | 8.32 ms |
| AWQ <br/> ExllamaV2 | 4   | 5.39 ms | 552  | 7.73 ms |
| AWQ <br/> ExllamaV2 | 4   | 5.51 ms | 8228 | 7.86 ms |
...
| 名称                             | BPW  | Prefill  | W    | B        |
|--------------------------------|------|----------|------|----------|
| 理论 w16kv16                     | 16   |          | 1321 | 24.74 ms |
| q8_0.gguf                      | 8.5  | 54.24 ms | 1046 | 22.79 ms | 
| q6_k.gguf                      | 6.95 | 45.75 ms | 1046 | 20.18 ms | 
| q5_k_m.gguf                    | 5.95 | 63.64 ms | 1048 | 18.37 ms | 
| q5_0.gguf                      | 5.56 | 58.25 ms | 1051 | 17.65 ms | 
| q4_k_m.gguf                    | 5.19 | 55.28 ms | 1050 | 17.08 ms | 
| q4_0.gguf                      | 4.62 | 49.15 ms | 1052 | 15.94 ms | 
| q3_k_m.gguf                    | 4.19 | 79.48 ms | 1054 | 15.46 ms | 
| q2_k.gguf                      | 3.43 | 81.11 ms | 1052 | 13.99 ms | 
| bfloat16                       | 16   | oom      | oom  | oom      |
| bnb int8                       | 8    | 3.76 ms  | 378  | 76.20 ms |
| bnb int4                       | 4    | 3.05 ms  | 355  | 16.09 ms |
| GPTQ-Int4 <br/> exllamav2 （默认) | 4    | 11.03 ms | 336  | 12.55 ms |

- 跟 4B 结论一样

| 名称                  | BPW | Prefill  | W    | B        |
|---------------------|-----|----------|------|----------|
| AWQ <br/> GEMM （默认) | 4   | 9.40 ms  | 334  | 14.09 ms |
| AWQ <br/> GEMM      | 4   | 6.33 ms  | 1080 | 12.29 ms |
| AWQ <br/> Exllama   | 4   | 11.40 ms | 334  | 12.86 ms |
| AWQ <br/> Exllama   | 4   | 6.11 ms  | 1086 | 11.21 ms |
| AWQ <br/> ExllamaV2 | 4   | 10.65 ms | 337  | 12.05 ms |
| AWQ <br/> ExllamaV2 | 4   | 5.36 ms  | 1087 | 10.80 ms |

| 名称                  | BPW | Prefill | W    | B        |
|---------------------|-----|---------|------|----------|
| AWQ <br/> GEMM （默认) | 4   | 8.29 ms | 334  | 14.15 ms |
| AWQ <br/> GEMM      | 4   | 6.11 ms | 1082 | 12.27 ms |
| AWQ <br/> Exllama   | 4   | 8.26 ms | 335  | 12.85 ms |
| AWQ <br/> Exllama   | 4   | 5.52 ms | 1083 | 11.24 ms |
| AWQ <br/> ExllamaV2 | 4   | 3.74 ms | 337  | 12.08 ms |
| AWQ <br/> ExllamaV2 | 4   | 5.98 ms | 1087 | 10.77 ms |


| 名称                  | BPW | Prefill  | W    | B        |
|---------------------|-----|----------|------|----------|
| AWQ <br/> GEMM （默认) | 4   | 9.68 ms  | 334  | 14.16 ms |
| AWQ <br/> GEMM      | 4   | 6.37 ms  | 6366 | 13.96 ms |
| AWQ <br/> Exllama   | 4   | 12.80 ms | 335  | 12.90 ms |
| AWQ <br/> Exllama   | 4   | 5.69 ms  | 6449 | 12.91 ms |
| AWQ <br/> ExllamaV2 | 4   | 8.48 ms  | 335  | 12.04 ms |
| AWQ <br/> ExllamaV2 | 4   | 6.19 ms  | 6385 | 12.45 ms |

### 6.4.6 Decoding 32B 

| 名称                                        | BPW  | Prefill   | W    | B         |
|-------------------------------------------|------|-----------|------|-----------|
| 理论 w16kv16                                | 16   |           | 4129 | 58.64 ms  |
| q8_0.gguf                                 | oom  | oom       | oom  | oom       | 
| q6_k.gguf                                 | oom  | oom       | oom  | oom       | 
| q5_k_m.gguf                               | 5.68 | oom       | oom  | oom       | 
| q5_0.gguf                                 | 5.53 | 136.64 ms | 1623 | 33.08 ms  | 
| q4_k_m.gguf                               | 4.85 | 115.86 ms | 1643 | 30.37 ms  | 
| q4_0.gguf                                 | 4.55 | 106.15 ms | 1566 | 29.08 ms  | 
| q3_k_m.gguf                               | 3.89 | 178.87 ms | 1562 | 26.55 ms  | 
| q2_k.gguf                                 | 3.01 | 208.12 ms | 1592 | 23.00 ms  | 
| bfloat16                                  | 16   | oom       | oom  | oom       |
| bnb int8                                  | 8    | oom       | oom  | oom       |
| bnb int4                                  | 4    | 2.71 ms   | 414  | 32.47 ms  |
| GPTQ-Int4 <br/> exllamav2 （默认)            | 4    | 1.75 ms   | 323  | 26.55 ms  |
| AWQ <br/> GEMM <br/>  fuse_layers False   | 4    | 1.83 ms   | 208  | 163.62 ms |
| AWQ <br/> Exllama <br/> fuse_layers False | 4    | 2.28 ms   | 819  | 167.67 ms |

| AWQ <br/> GEMM <br/>  fuse_layers False   | 4    | 2.23 ms   | 252  | 166.70 ms |
| AWQ <br/> Exllama <br/> fuse_layers False | 4    | 1.69 ms   | 301  | 165.80 ms |


max_seq_len no
| AWQ <br/> GEMM <br/>  fuse_layers False   | 4    | 2.32 ms   | 287  | 167.35 ms |
| AWQ <br/> Exllama <br/> fuse_layers False | 4    | 1.98 ms   | 296  | 165.44 ms |
